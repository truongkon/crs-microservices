...
export function useCourses(keyword: string, page: number, size = 10) {
    ...
    useEffect(() => {
        // eslint-disable-next-line react-hooks/set-state-in-effect -- fetch du lieu khi mount/keyword-page-size thay doi la pattern hop le, setState('loading') chay truoc await nen khong gay cascading render thuc su
        fetchCourses();
    }, [fetchCourses]);
